...
class TitleState extends MusicBeatState
{
	...
	public static var cachedChars:Array<Character> = [];
	...
	override public function create():Void
	{
		...
		#else
		if (!initialized) bop(); else startIntro();
		#end
	}
	...
	function bop() // preload the game, ya know, shit like that
	{
		Sys.command('echo Wait until your game loades...');
		for (koo in sys.FileSystem.readDirectory(Sys.getCwd() + 'assets/characters'))
		{
			trace('loading $koo');
			var pu:Character = new Character(-1000, -1000, koo.split('.')[0]);
			add(pu);
			pu.setPosition(0,0);
			cachedChars.push(pu);
			pu.setPosition(-1000,-1000);
		}
		FlxG.sound.cacheAll();
		trace('loaded!');
		startIntro();
	...
	override function update(elapsed:Float)
	{
		...
		if (!transitioning && skippedIntro)
		{
			if(pressedEnter)
			{
				...
				FlxTween.tween(FlxG.camera, {y: 1000}, {ease: FlxEase.quadInOut, onComplete: function (haha) {
