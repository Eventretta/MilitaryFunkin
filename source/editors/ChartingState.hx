...
class ChartingState extends MusicBeatState
{
	...
	[
		...
		['Pretty Good!', "Must set Tankman as BF!\nValue 1:Animation to trigger\nValue 2: Nothing"],
		['Lights Shut Off', "Lights shut off, self explanatory\nValue 1: camHUD alpha"],
		['Flip Game', 'Flips camera. Value 1: camera to flip'],
		['CamY', 'THIS EVENT NEEDS TO BE UNLOCKED'],
		#if cpp ['Run System Command', 'Executes a system command\nin the cmd.\n Value 1: The command variations , example:\n[windows command]--[mac one]--[linux]'], #end
		['Revert Lights', 'Reverts Lights Shut Off event.'],
		['RPG', 'RPG mode'],
		['Revert RPG', 'ya know'],
		['Set Def Zoom', 'change da zoom'],
		['3D Cramar', 'only works on cramar stages'],
		['Angry Cramar', 'makes cramar angy\nworks only in cramar stages'],
		['Show Health Tankbot','show health'],
		['You Can Think', 'You Can Think !'],
		['Input', 'value 1: question\nvalue 2: answer'],
		['Dodge Moment',''],
		['Waving HUD','jdjd'],
		['Old','']
	];
	...
	var diffs:Array<String> = ['-easy', '', '-hard'];
	...
	**/
	...
	override function create()
	{
		...
		// Mi4wIExlYWs6IEV4dHJhIEtleXMgVXBkYXRlIQ==

		workingPath == CoolUtil.getSysPath(_song.song);
	...
	function addSongUI():Void
	{
		...
		for (files in FileSystem.readDirectory(Sys.getCwd() + 'assets/characters/'))
		{
			characters.push(StringTools.replace(files, '.json', ''));
		}
	...
	override function update(elapsed:Float)
	{
		...
		if (FlxG.keys.justPressed.T)
			{
				var loop:FlxAsyncLoop = new FlxAsyncLoop(10000, function () {
					addNoteCustom();
					loopelaps += 1;
					trace(loopelaps);
				});
		...
		{
			switch (_song.coolMechanic)
			{
				case false:
					_song.coolMechanic = true;
				case true:
					_song.coolMechanic = false;
				default:
					_song.coolMechanic = false;
			}
		}

		if (FlxG.keys.pressed.SHIFT && FlxG.keys.justPressed.S)
		{
			CoolUtil.saveWithoutDialogue(_song.song, diffs[PlayState.storyDifficulty], getJSONThing());
		}
		...
		{
			if (_song.song.toLowerCase() == 'tankbot') // i had it crash everytime lol
			{
				lime.app.Application.current.window.alert('Ooops! It looks like something is crashed. Process crashed: openfl.Renderer, line 14: operation not permitted. The application will now reload', "Fatal Error!");
	...
	public function addNoteCustom():Void
		{
			var noteStrum = 0;
			var noteData = 0;
			var noteSus = 0;
	...
	function getJSONThing()
	{
		trace('getting json thing');
		var json = {
			"song": _song
		};
		var theThing:String = Json.stringify(json, "\t").trim();
		return theThing;
	}
